...
const ClientForm = ({ setDisplay, data, setData }) => {
  const [state, setState] = useState({
	...
	useEffect(() => {
		const gets = async() => {
			axios.get(urls.industry)
			.then(res => setindustryOptions(res.data))
			.catch(err => (console.error(err)))
			await axios.get(urls.stack)
			.then(res => setStackOptions(res.data))
			.catch(err => (console.error(err)))
			axios.get(urls.seniority)
			.then(res => setSeniorityOptions(res.data.map((option) => ({level: option}))))
			.catch(err => (console.error(err)))
			await axios.get(urls.area)
